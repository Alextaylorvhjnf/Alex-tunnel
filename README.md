...
| ویژگی | توضیحات |
|--------|-----------|
| راه اندازی فوق سریع | تنها با چند مرحله ساده، تونل Reverse یا Direct (ایران به خارج) را ایجاد کنید |
...
| دسترسی آسان | پس از اولین اجرا، اسکریپت از طریق دستور ALEX در دسترس خواهد بود |
...
| پشتیبانی از HAProxy | لود بالانسینگ خودکار برای تونل های Direct |

---

## دریافت و اجرای اسکریپت

### روش 1: اجرای سریع (توصیه شده)
...
### روش 5: آپلود با SFTP (برای سرورهای بدون دسترسی به گیت هاب)
...
ابتدا هسته ALEX را با گزینه 3 روی هر دو سرور نصب کنید (اگر در ایران با خطا مواجه شد، فایل را دانلود کرده، با SFTP آپلود و استخراج نمایید).

---

### مرحله 1: پیکربندی سرور خارج (Kharej)

1. گزینه 1 (Direct) را انتخاب کنید
2. گزینه "Server Kharej" (سرور خارج) را بزنید
3. یک نام برای تونل وارد کنید (مثلا Test2)
4. نوع IP را انتخاب کنید (IPv4 یا IPv6)
5. IP سرور ایران را وارد کنید
6. پورت تونل را وارد کنید (مثلا 4790) - **توجه: این پورت کانفیگ نیست**
7. یک مقدار برای VXLAN ID مشخص کنید (مثلا 100) - همین مقدار را در سرور ایران هم استفاده کنید
8. یک IP بریج برای سرور خارج وارد کنید (مثلا 10.0.10.2) - این IP در سرور ایران مورد نیاز خواهد بود
...
3. یک نام برای تونل وارد کنید (مثلا Test2) - **دقیقا همان نام مرحله قبل**
4. نوع IP را انتخاب کنید (IPv4 یا IPv6)
5. IP سرور خارج را وارد کنید
6. پورت تونل را وارد کنید (همان پورتی که در سرور خارج تنظیم کردید)
7. یک IP بریج برای سرور ایران وارد کنید (مثلا 10.0.10.1)
8. IP بریج سرور خارج را وارد کنید (مثلا 10.0.10.2) - **همان IP مرحله 8 سرور خارج**
9. همان VXLAN ID تنظیم شده در سرور خارج را وارد کنید
10. پورت های کانفیگ خود را وارد کنید (مثلا 8080,40001)
...
1. گزینه 2 (Reverse) را انتخاب کنید
...
| مدیریت خطا | اسکریپت به طور خودکار دو خطای متوالی در لاگ ها را تشخیص داده و تونل را ریستارت می کند |
| امنیت | حتما توکن امنیتی پیش فرض را تغییر دهید |
...
| مشکل | راه حل |
|------|---------|
| خطای 404 در نصب | مطمئن شوید فایل alex_manager.sh در ریشه ریپازیتوری قرار دارد |
...
| سرویس start نمیشود | journalctl -u ALEX-* -f |
| VXLAN کار نمیکند | مطمئن شوید پورت UDP برای تونل مستقیم باز است |

---

## پشتیبانی

- مخزن گیت هاب: https://github.com/Alextaylorvhjnf/Alex-tunnel
...
اگر این پروژه برای شما مفید بوده است، میتوانید از طریق آدرس زیر برای حمایت مالی اقدام کنید:
